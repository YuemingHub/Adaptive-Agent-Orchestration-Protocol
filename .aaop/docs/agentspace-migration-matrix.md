...
| **GATE** (`COMMANDER \| FOUNDER`; human-authority escalation only) | `decision_ownership` (`human_owned` / `agent_owned` / `joint`) + `task_pod_policy.independent_review_when_consequential` + `role_provider_order` | AAOP_HAS_EQUIVALENT | Retire. AAOP expresses "who owns advance/review authority" as `decision_ownership`; the Founder-Gate trigger maps to `human_owned` items + `human_open_questions`. |
| **STOP-WHEN** (autonomous terminal criterion) | `autonomy.md` policy + `alignment.success_evidence` + route `Outcome`; no literal `stop_when` field | AAOP_HAS_EQUIVALENT (policy-level) | Retire. If a machine-readable field is wanted later, add optional `stop_when` to `working-contract.schema.json` — but the *semantic* is already covered by the autonomy policy, so no migration is required now. |
| **RETURN-WHEN** (escalate-with-evidence) | `human_open_questions` + `blockers` + `risks` + `Decision Frame` | AAOP_HAS_EQUIVALENT | Retire. Same note: a literal `return_when` field is optional polish, not a missing semantic. |
