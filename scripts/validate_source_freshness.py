...
def main() -> int:
    ...
    require(bool(version), "authoritative AAOP VERSION must be non-empty")
    stale_fixture_version = "1.0.0" if version != "1.0.0" else "0.9.0"
    ...
    require(
        f"**v{version} — production release line" in readme,
        f"README status must identify authoritative package version v{version}",
    )
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="aaop-source-freshness-") as tmp:
            ...
            current = write_package(root / "current", version=version, kind="official-ref", ref="stable")
            tool.fetch_stable_version = lambda timeout=5.0: version
            ...
            stale = write_package(root / "stale", version=stale_fixture_version, kind="official-ref", ref="stable")
            report = tool.inspect(stale)
            require(report["state"] == "stale", f"stale stable classification failed: {report}")
            require(report["stable_version"] == version, f"stale report missing stable identity: {report}")
            ...
            offline_package = write_package(root / "offline", version=version, kind="official-ref", ref="stable")
            ...
            exact = write_package(root / "exact", version=stale_fixture_version, kind="official-ref", ref="a" * 40)
            ...
            local = write_package(root / "archive", version=stale_fixture_version, kind="local-archive", ref=None)
    ...
    print(
        f"PASS source freshness: v{version} release identity is authoritative; stable-managed current/stale/offline and exact-frozen/local policies remain separate"
    )
    return 0
